...
namespace Easytl.SafeHelper
{
    ...
    /// BlowFish加密
    /// </summary>
    public class BlowFishCryptoServiceProvider
    {
        ...
        public BlowFishCryptoServiceProvider(string hexKey)
        {
        ...
        public BlowFishCryptoServiceProvider(byte[] cipherKey)
        {
            randomSource = new RNGCryptoServiceProvider();
            SetupKey(cipherKey);
        }
        ...
        {
            uint x1 = (bf_s0[wordByte0(0xB)] + bf_s1[wordByte1(0xB)]) ^ bf_s2[wordByte2(0xB)];
            uint x2 = x1 + bf_s3[this.wordByte3(0xB)];
            uint x3 = x2 ^ bf_P[n];
            return x3 ^ a;
        }
