...
namespace Easytl
{
    public static class Lunisolar
    {
        ...
        ///<return s></return s>
        public static string GetChineseDateTime(this DateTime datetime, bool hasYear = true, bool hasMonth = true, bool hasDay = true)
        {
            ...
            StringBuilder LunisolarStr = new StringBuilder();
            if (hasYear)
            {
                LunisolarStr.Append(GetLunisolarYear(year));
                LunisolarStr.Append("年");
            }
            if (hasMonth)
            {
                LunisolarStr.Append(isleap ? "闰" : string.Empty);
                LunisolarStr.Append(GetLunisolarMonth(month));
                LunisolarStr.Append("月");
            }
            if (hasDay)
                LunisolarStr.Append(GetLunisolarDay(day));

            return LunisolarStr.ToString();
        }
    }
}
